...
function fileUpload($files, $uploadFileName = "img", $uploadFolder = "", $ext_array = array(), $optionsUpload = array()) {
    if (fnIsNotNull($ext_array)) {
        foreach ($ext_array as &$ea) {
            $ea = strtolower($ea);
        }
    }
    // Save the uploaded files
    if (!is_array($files['tmp_name']['0'])) {
        $files_temp = array();
        $files_temp['name'][0] = $files['name'];
        $files_temp['type'][0] = $files['type'];
        $files_temp['tmp_name'][0] = $files['tmp_name'];
        $files_temp['error'][0] = $files['error'];
        $files_temp['size'][0] = $files['size'];
        $files = $files_temp;
    }
    $return_data = array();
    ...
    for ($index = 0; $index < count($files['name']); $index++) {
        $fileArr = array();
        $fileArr['name'] = $files['name'][$index];
        $fileArr['type'] = $files['type'][$index];
        $fileArr['tmp_name'] = $files['tmp_name'][$index];
        $fileArr['error'] = $files['error'][$index];
        $fileArr['size'] = $files['size'][$index];
        ...
        if (fnIsNull(in_array($extension, $ext_array)) && fnIsNotNull($ext_array)) {
            $return_data['error'][] = "Only following extensions are allowed: <b>" . implode(",", $ext_array) . "</b>";
            continue;
        }
        if (fnIsNull($uploadFileName)) {
            $uploadFileName = $fileArr['name'];
        }
        $uploadFileName = str_ireplace("." . $extension, "", $uploadFileName) . "." . $extension;

        if (fnIsNull($uploadFolder)) {
            $uploadFolder = UPLOAD_PATH;
        }
        $uploadFolder .= "/";
        $uploadFolder = rtrim($uploadFolder, "/") . "/";
        $uploadFileName = uniqueFilenameDP($uploadFolder . $uploadFileName);
        $uploadPathWitFileName = $uploadFolder . $uploadFileName;
        $isUpload = fileCopyMove($file, $uploadPathWitFileName, $optionsUpload);

        if (isset($isUpload['success'])) {
            $arr = array();
            $arr['filename'] = $uploadFileName;
            $arr['extension'] = $extension;
            $arr['uploadFolder'] = $uploadFolder;
            $arr['imagePath'] = $uploadPathWitFileName;
            $arr['success'] = $isUpload['success'];
            $return_data['success'][] = $arr;
        } else {
            $return_data['error'][] = array_unique($isUpload);
        }
    }
    if (isset($return_data['error']) && fnIsNotNull($return_data['error'])) {
        // $allerror = call_user_func_array('array_merge', $return_data['error']);
        $er = array();
        foreach ($return_data['error'] as $e) {
            if (is_array($e)) {
                $a = array_values($e);
            } else {
                $a = $e;
        ...
        $er = array_unique($er);
        $return_data['error'] = $er;
    }
    return $return_data;
}

function mkdirDP($pathname, $mode = 0777) {
    @chmod($pathname, $mode);
    is_dir(dirname($pathname)) || mkdirDP(dirname($pathname), $mode);
    return is_dir($pathname) || @mkdir($pathname, $mode);
}

function uniqueFilenameDP($uploadPathWitFileName, $returnFullPath = 0) {

    $info = pathinfo($uploadPathWitFileName);
    $dir = (!empty($info['dirname']) && $info['dirname'] != ".") ? $info['dirname'] : '';
...
//uploadorg = true  or array("width" => 100, "height" => 100) or null
//$options = array() or NULL or array(uploadorg=>null,'size1' => array("width" => 500, "height" => 500))
function fileCopyMove($source, $dest, $options = array()) {
    ...
    if (is_file($source)) {
        ...
        if (isset($options) && is_array($options) && count($options)) {
            if (isset($options['cropImageOption']) && is_array($options['cropImageOption'])) {
                $w = $options['cropImageOption'][0];
                $h = $options['cropImageOption'][1];
                $x = $options['cropImageOption'][2];
                $y = $options['cropImageOption'][3];
                unset($options['cropImageOption']);
                $imDimention = $x . "x" . $y;
                $magicianObj = new imageLib($source);
                $result = false;
                if (count($magicianObj->getErrors()) <= 0) {
                    $magicianObj->cropImage($w, $h, "$imDimention");
                    $cropImageDir = $dir . '/';
                    if (!is_dir($cropImageDir)) {
                        @mkdir($cropImageDir, 0777, true);
                    }
                    @chmod($cropImageDir, 0777);

                    $cropImageDir = $cropImageDir . $curFilename;
                    $result = $magicianObj->saveImage($cropImageDir, 100);
                    if ($result) {
                        $source = $cropImageDir;
                    }
                }
            ...
            foreach ($options as $k => $v) {
                ...
                if (!is_dir($orgDir)) {
                    @mkdir($orgDir, 0777, true);
                }
                @chmod($orgDir, 0777);
                $orgDir = $orgDir . $curFilename;
                $result = false;
                if (is_array($v) && count($v) && $v['width'] && $v['height']) {
                    $magicianObj = new imageLib($source);
                    if (count($magicianObj->getErrors()) <= 0) {
                        $magicianObj->resizeImage($v['width'], $v['height'], $cropOption);
                        $result = $magicianObj->saveImage($orgDir, 100);
                    }
                } else {
                    $result = copy($source, $orgDir);
                }
                if ($result) {
                    $return_arr[$k] = $orgDir;
                    @chmod($orgDir, 0777);
    ...
    if (count($error)) {
        ...
        return array('success' => $return_arr);
    }
}
